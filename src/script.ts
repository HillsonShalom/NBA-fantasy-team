import { populateTable, teamInit } from "./components";
import { searchClass, searchModel } from "./models";
...
function initPage(): void {
    ...
    submit.addEventListener('click', e => {
        e.preventDefault();
        const searchData: searchClass = new searchClass(select.value, Number(twoRange.value), Number(threeRange.value), Number(pointsRange.value));
...
let a: searchClass = new searchClass("C", 20, 20 ,100);
